...
// Multer konfigürasyonu - hero görselleri için
const heroStorage = multer.diskStorage({
  destination: (req, file, cb) => {
    const uploadDir = 'uploads/hero-images'
...
function mapHomepageSettings(settings) {
  ...
  return {
    hero: {
      ...
      backgroundImage: settings.homepageHeroBackgroundImage ?? defaultHomepage.hero.backgroundImage,
      backgroundOpacity: settings.homepageHeroBackgroundOpacity ?? defaultHomepage.hero.backgroundOpacity
    },
  ...
  try {
    ...
    const settings = await prisma.settings.upsert({
      ...
      update: {
        ...
        enableBiddingHours: enableBiddingHours !== undefined ? enableBiddingHours : false,
...
router.post('/homepage', requireAdmin, async (req, res) => {
  try {
    ...
    const sanitizeOpacity = (value, fallback = 0.75) => {
      const num = typeof value === 'number' ? value : parseFloat(value)
      if (isNaN(num) || num < 0 || num > 1) return fallback
      return num
    }
    ...
    const updateData = {
      ...
      homepageHeroBackgroundImage: sanitizeString(hero.backgroundImage, defaultHomepage.hero.backgroundImage),
      homepageHeroBackgroundOpacity: sanitizeOpacity(hero.backgroundOpacity, defaultHomepage.hero.backgroundOpacity),
