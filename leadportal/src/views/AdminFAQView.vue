  ...
  try {
    ...
    if (data.faqs && Array.isArray(data.faqs)) {
      for (const faq of data.faqs) {
        if (faq.id) {
          await axios.put(`faq/admin/${faq.id}`, faq)
        } else {
          await axios.post('faq/admin', faq)
        }
      }
    }
...
const { 
  ...
  getData: getAllFAQsData,
  setData: setAllFAQsData,
  validateData: validateFAQsData,
  fileName: 'faq-settings',
  fileExtension: 'json'
})
